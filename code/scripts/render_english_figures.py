...
def figure1() -> None:
    ...
    ax = axes[0]
    ax.bar(years, contracts, color="#1f6acb", width=0.78)
    ax.set_ylim(0, 65000)
    ...
    ax = axes[1]
    ax.bar(years, values, color="#ef6100", width=0.78)
    ax.set_ylim(0, 6.4)
    ax.set_yticks([0, 2, 4, 6])
    ax.yaxis.set_major_formatter(FuncFormatter(lambda value, _pos: f"${int(value)}B"))
    ax.set_xticks(years)
    ax.set_xticklabels([str(y) for y in years])
...
def figure2() -> None:
    ...
    ax_left.set_xticklabels(["$0MM", "$1000MM", "$3000MM", "$5000MM", "$7000MM", "$9000MM"])
...
def figure4() -> None:
    ...
    ax_l.text(3.2, 88, "Top 3% = 85.9%", color="#d82d2d", fontsize=11)
    ax_l.set_xlim(0.01, 100)
    ax_l.set_ylim(0, 100)
    ax_l.set_xticks([0.01, 0.1, 1, 10, 100])
    ax_l.set_xticklabels(["1%", "", "10%", "", "100%"])
    ...
    ax_r.legend(["% of total value"], frameon=False, loc="upper center", bbox_to_anchor=(0.48, 0.93), fontsize=11, labelcolor="#333333", handlelength=1.2)
